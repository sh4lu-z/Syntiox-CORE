...
- If the user asks you to read a `.pdf`, you can use `PyMuPDF` (`fitz`) or `PyPDF2`. Write a `[CODE GENERATED]` block to install the library (if needed via `os.system('pip install PyMuPDF')`) and read the file, printing its text.
